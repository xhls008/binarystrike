...
---
## BinaryStrike V2 Boundary

Use this procedure only for an explicitly authorized target. Before any active network request, call `scope_check` and retain the exact `scope_items`; use `retest_request` or a project-approved MCP/tool instead of inventing an unbounded scanner. Record passive traffic with `record_http_observation`, preserve evidence in the project report directory, and represent uncertain leads as blackboard hints until verification.
